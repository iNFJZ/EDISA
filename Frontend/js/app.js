...
window.deleteUser = async function (id) {
  if (confirm(window.i18next.t("confirmDeleteUser"))) {
    try {
      ...
      showToastrMessage(window.i18next.t("userDeletedSuccessfully"), "success");
      ...
      if (
        ...
        showToastrMessage(window.i18next.t("yourAccountHasBeenDeleted"), "info");
        setTimeout(() => {
      ...
      if (
        ...
        showToastrMessage(window.i18next.t("deactivationEmailSent"), "info");
      }
      renderUserTable();
    } catch (error) {
      showToastrMessage(window.i18next.t("failedToDeleteUser"), "error");
    }
  }
};
    ...
    try {
      ...
      if (res.ok && data.token) {
        ...
        if (errorMsg.toLowerCase().includes("banned")) {
          ...
          showToastrMessage(errorMsg, "error");
      ...
      if (msg) msg.textContent = errorMsg;
      showToastrMessage(window.i18next.t("googleLoginFailed"), "error");
    }
    if (!success && msg) {
...
function setupLogoutHandlers() {
  ...
  if (logoutBtn && !logoutBtn.hasAttribute("data-logout-handler")) {
    ...
    logoutBtn.addEventListener("click", async function (e) {
      ...
      currentUser = null;
      authToken = null;
      showToastrMessage(window.i18next.t("loggedOutSuccessfully"), "success");
      setTimeout(() => {
        window.location.href = "/auth/login.html";
      }, 500);
    });
...
window.addEventListener("DOMContentLoaded", async function () {
  try {
    ...
    showToastrMessage(window.i18next.t("failedToInitializeApplication"), "error");
  }
});
...
async function loadDeletedUsers() {
  const deletedUserTableBody = document.querySelector(
    "#deleted-user-table tbody",
  );
  const deletedSearchInput = document.getElementById(
    "deleted-user-search-input",
  );
  const deletedPaginationContainer = document.getElementById(
    "deleted-pagination-container",
  );
  ...
  try {
    ...
    renderDeletedUserTableWithPagination(
  ...
  if (deletedSearchInput) {
    deletedSearchInput.addEventListener("input", function () {
      const keyword = this.value.trim().toLowerCase();
      filteredDeletedUsersCache = !keyword
        ? allDeletedUsersCache
        : allDeletedUsersCache.filter(
            (u) =>
              (u.username && u.username.toLowerCase().includes(keyword)) ||
              (u.email && u.email.toLowerCase().includes(keyword)) ||
              (u.fullName && u.fullName.toLowerCase().includes(keyword)),
...
function renderUserTableWithPagination(users, page) {
  ...
  userTableBody.innerHTML = pageUsers
    .map(
      (user, idx) => `
  ...
  document.querySelectorAll(".btn-delete-user").forEach((btn, idx) => {
    btn.onclick = async function (e) {
      ...
      try {
        ...
        showToastrMessage(window.i18next.t("deleteFailed"), "error");
        btn.disabled = false;
...
function renderDeletedUserTableWithPagination(users, page) {
  ...
  document.querySelectorAll(".btn-restore-user").forEach((btn, idx) => {
    btn.onclick = async function (e) {
      ...
      const user = pageUsers[idx];
      if (
        confirm(`${window.i18next.t("confirmRestoreUser")} "${user.username}"?`)
      ) {
        btn.disabled = true;
        btn.textContent = window.i18next.t("restoring");
        try {
          await restoreUser(user.id);
          showToastrMessage(window.i18next.t("userRestoredSuccessfully"), "success");
          allUsersCache = [];
          filteredUsersCache = [];
          allDeletedUsersCache = [];
          filteredDeletedUsersCache = [];
          ...
          showToastrMessage(window.i18next.t("restoreFailed"), "error");
          btn.disabled = false;
...
window.addEventListener("DOMContentLoaded", function () {
  ...
  if (editForm) {
    editForm.onsubmit = async function (e) {
      ...
      if (errors.length > 0) {
        showToastrMessage(errors.filter(Boolean).join("<br>"), "error");
        return;
      }
      ...
      try {
        ...
        showToastrMessage(window.i18next.t("userUpdatedSuccessfully"), "success");
        closeEditModal();

        const oldUser = allUsersCache.find((u) => u.id === userId);
        if (
          oldUser &&
          (oldUser.isVerified !== isVerified ||
            oldUser.status !== parseInt(status))
        ) {
          allUsersCache = [];
          filteredUsersCache = [];
          allDeletedUsersCache = [];
...
function editUser(userId) {
  ...
  let statusVal = [1, 2, 3, 4].includes(Number(user.status))
    ? String(user.status)
    : "2";
  ...
  const modal = document.getElementById("edit-user-modal");
  if (!modal) {
    showToastrMessage(window.i18next.t("modalNotFound"), "error");
    return;
  }
...
function showUserDetailModal(user) {
  ...
  content.innerHTML = `
      ...
      <div><b>${window.i18next.t("status")}:</b> ${(() => {
        switch (user.status) {
          case 1:
          ...
          default:
            return sanitizeHtml(user.status || "");
        }
      })()}</div>
...
window.addEventListener("DOMContentLoaded", function () {
  ...
  if (userDetailModal && closeUserDetailBtn) {
    ...
    document.addEventListener("keydown", function (event) {
      if (
        event.key === "Escape" &&
        userDetailModal.classList.contains("show")
      ) {
        closeUserDetailModal();
      }
    });
